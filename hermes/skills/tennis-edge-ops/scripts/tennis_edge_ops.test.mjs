...
test("source-discovery maps safe data acquisition paths without bypasses or live calls", async () => {
  ...
  try {
    ...
    assert.equal(payload.discovery_scope.includes("cc0_historical_match_backfill"), true);
    assert.equal(payload.discovery_scope.includes("point_by_point_or_shot_backfill"), true);
    assert.equal(payload.discovery_scope.includes("licensed_historical_odds_archive"), true);
    ...
    assert.equal(payload.backfill_source_summary.cc0_candidates, 1);
    assert.equal(payload.backfill_source_summary.licensed_api_candidates, 1);
    const dossierById = Object.fromEntries(payload.backfill_source_dossier.map((source) => [source.id, source]));
    assert.equal(dossierById.match_charting_project.license_class, "cc_by_nc_sa_operator_review");
    assert.equal(dossierById.match_charting_project.feature_targets.includes("serve_pattern_priors"), true);
    assert.equal(dossierById.kaggle_tennis_cc0_candidates.license_class, "cc0_candidate_verify");
    assert.equal(dossierById.theoddsapi_archive.license_class, "paid_api_terms");
    assert.equal(dossierById.jeff_sackmann_slam_pointbypoint.prohibited_use.includes("live_point_feed_replacement"), true);
...
test("historical-backfill-plan ranks allowed historical sources without fetching them", async () => {
  ...
  try {
    ...
    assert.equal(payload.review_summary.total_sources >= 8, true);
    assert.equal(payload.review_summary.license_review_required >= 7, true);
    ...
    assert.equal(byId.match_charting_project.feature_targets.includes("serve_pattern_priors"), true);
    assert.equal(byId.match_charting_project.prohibited_use.includes("live_match_scraping"), true);
    assert.equal(byId.kaggle_tennis_cc0_candidates.license_class, "cc0_candidate_verify");
    assert.equal(byId.kaggle_tennis_cc0_candidates.required_operator_evidence.includes("dataset_id"), true);
    assert.equal(byId.theoddsapi_archive.license_class, "paid_api_terms");
    assert.equal(byId.theoddsapi_archive.prohibited_use.includes("quota_spend_without_operator"), true);
...
test("source-intake-plan turns manifest and ledger evidence into safe intake queues", async () => {
  ...
  try {
    ...
    const liveStatsContract = payload.intake_contracts.find((contract) => contract.id === "live_stats_feature_intake_contract");
    assert.equal(Boolean(liveStatsContract), true);
    assert.equal(liveStatsContract.feature_contract_id, "live_stats_feature_contract");
    assert.equal(liveStatsContract.input_contracts.includes("ProviderCursors"), true);
    assert.equal(liveStatsContract.output_contracts.includes("LiveFeatureSnapshotSeed"), true);
    assert.equal(liveStatsContract.output_contracts.includes("CollectionCadenceSeed"), true);
    assert.equal(liveStatsContract.output_contracts.includes("SignalGateContextSeed"), true);
    assert.equal(liveStatsContract.output_contracts.includes("LearningReviewSeed"), true);
    assert.equal(liveStatsContract.validation_command, "npm --silent run hermes:live-stats");
    assert.equal(liveStatsContract.provider_api_call_allowed, false);
    assert.equal(liveStatsContract.live_api_calls, false);
    assert.equal(liveStatsContract.dataset_fetch_allowed, false);
    assert.equal(liveStatsContract.browser_scraping_allowed, false);
    assert.equal(liveStatsContract.can_submit_real_orders, false);
    assert.equal(liveStatsContract.llm_per_tick_allowed, false);
...
test("experiment-lab surfaces source-intake backlog evidence", async () => {
  const tempDir = mkdtempSync(join(tmpdir(), "tennis-edge-experiment-lab-source-intake-"));
  const fakeHermes = join(tempDir, "hermes-fake.mjs");
  const sourceIntakeLedgerPath = join(tempDir, "source-intake-ledger.jsonl");
  writeFileSync(
    fakeHermes,
  ...
  try {
    ...
    assert.equal(payload.can_submit_real_orders, false);
    assert.equal(payload.can_create_paper_orders, false);
    assert.equal(payload.backlog_guidance.next_item_id, "harden_source_intake_feedback_loop");
    assert.equal(payload.backlog_guidance.next_item_source.includes("source_intake_ledger"), true);
    const byId = Object.fromEntries(payload.experiments.map((experiment) => [experiment.id, experiment]));
    assert.equal(byId.source_intake_feedback_loop.command, "npm --silent run hermes:source-intake-ledger-report");
    assert.equal(byId.source_intake_feedback_loop.executes_now, false);
    assert.equal(byId.source_intake_feedback_loop.provider_api_call_allowed, false);
    assert.equal(byId.source_intake_feedback_loop.can_submit_real_orders, false);
    assert.equal(byId.source_intake_feedback_loop.evidence.includes("backlog_next_item=harden_source_intake_feedback_loop"), true);
  } finally {
    server.close();
  }
});
...
test("backlog-plan uses source-intake ledger as implementation evidence", async () => {
  const tempDir = mkdtempSync(join(tmpdir(), "tennis-edge-backlog-source-intake-plan-"));
  const experimentLedgerPath = join(tempDir, "experiment-ledger.jsonl");
  const operatorLedgerPath = join(tempDir, "operator-ledger.jsonl");
  const missionLedgerPath = join(tempDir, "mission-ledger.jsonl");
  const controllerLedgerPath = join(tempDir, "live-controller-ledger.jsonl");
  ...
  assert.equal(payload.can_submit_real_orders, false);
  assert.equal(payload.can_create_paper_orders, false);
  assert.equal(payload.items[0].id, "harden_source_intake_feedback_loop");
  assert.equal(payload.items[0].source.includes("source_intake_ledger"), true);
  assert.equal(payload.items[0].frequency, 2);
  assert.equal(payload.items[0].validation_commands.includes("npm --silent run hermes:source-intake-ledger-report"), true);
  ...
  assert.equal(payload.items[0].acceptance_evidence.includes("dataset_fetch_attempted_count=0"), true);
  assert.equal(payload.items[0].acceptance_evidence.includes("provider_command_executed_count=0"), true);
  assert.equal(payload.items[0].acceptance_evidence.includes("bypass_attempted_count=0"), true);
  assert.equal(payload.items[0].executes_now, false);
  ...
  assert.equal(payload.evidence.source_intake_ledger.allowed_contract_counts[0].count, 2);
  assert.equal(payload.evidence.source_intake_ledger.operator_review_counts[0].count, 2);
  assert.equal(payload.evidence.source_intake_ledger.deferred_counts[0].count, 2);
  assert.equal(payload.evidence.source_intake_ledger.forbidden_quarantine_counts[0].count, 2);
  assert.equal(payload.evidence.source_intake_ledger.intake_command_executed_count, 0);
  assert.equal(payload.evidence.source_intake_ledger.dataset_fetch_attempted_count, 0);
  assert.equal(payload.evidence.source_intake_ledger.provider_command_executed_count, 0);
  assert.equal(payload.evidence.source_intake_ledger.bypass_attempted_count, 0);
  assert.equal(payload.safety.can_submit_real_orders, false);
});
...
test("autonomy-effectiveness includes repeated source-intake pressure", async () => {
  const tempDir = mkdtempSync(join(tmpdir(), "tennis-edge-autonomy-effectiveness-source-intake-"));
  const experimentLedgerPath = join(tempDir, "experiment-ledger.jsonl");
  const operatorLedgerPath = join(tempDir, "operator-ledger.jsonl");
  const missionLedgerPath = join(tempDir, "mission-ledger.jsonl");
  const controllerLedgerPath = join(tempDir, "live-controller-ledger.jsonl");
  ...
  assert.equal(payload.can_create_paper_orders, false);
  assert.equal(payload.evidence_totals.total_records, 2);
  assert.equal(payload.evidence_totals.source_intake_records, 2);
  assert.equal(payload.protected_action_claims.total, 0);
  assert.equal(payload.protected_action_claims.intake_commands, 0);
  assert.equal(payload.protected_action_claims.dataset_fetches, 0);
  assert.equal(payload.protected_action_claims.provider_commands, 0);
  assert.equal(payload.protected_action_claims.bypass_attempts, 0);
  assert.equal(payload.repeat_pressure.source_intake, 2);
  assert.equal(payload.effectiveness_matrix.source_intake.status, "repeated_blocker");
  assert.equal(payload.effectiveness_matrix.source_intake.top_signal, "route:replay_backfill");
  assert.equal(payload.effectiveness_matrix.source_intake.command, "npm --silent run hermes:replay-backfill-contract");
  assert.equal(payload.backlog_feedback.next_item.id, "harden_source_intake_feedback_loop");
  assert.equal(payload.next_action.id, "harden_source_intake_feedback_loop");
  assert.equal(payload.next_action.command, "npm --silent run hermes:implementation-handoff");
  assert.equal(payload.next_action.executes_now, false);
  assert.equal(payload.safety.can_submit_real_orders, false);
});
...
test("implementation-handoff turns source-intake pressure into a safe work order", async () => {
  const tempDir = mkdtempSync(join(tmpdir(), "tennis-edge-implementation-source-intake-"));
  const experimentLedgerPath = join(tempDir, "experiment-ledger.jsonl");
  const operatorLedgerPath = join(tempDir, "operator-ledger.jsonl");
  const missionLedgerPath = join(tempDir, "mission-ledger.jsonl");
  const controllerLedgerPath = join(tempDir, "live-controller-ledger.jsonl");
  ...
  assert.equal(payload.can_submit_real_orders, false);
  assert.equal(payload.can_create_paper_orders, false);
  assert.equal(payload.source_plan.next_item_id, "harden_source_intake_feedback_loop");
  assert.equal(payload.work_order.id, "harden_source_intake_feedback_loop");
  assert.equal(payload.work_order.executes_now, false);
  assert.equal(payload.work_order.provider_api_call_allowed, false);
  assert.equal(payload.work_order.can_submit_real_orders, false);
  assert.equal(payload.work_order.can_create_paper_orders, false);
  assert.equal(payload.work_order.suggested_steps.includes("review_source_intake_ledger_report_for_allowed_operator_deferred_and_forbidden_queues"), true);
  assert.equal(payload.work_order.suggested_steps.includes("map_the_top_allowed_contract_to_replay_or_internal_fastapi_read_model_without_fetching_data"), true);
  assert.equal(payload.work_order.suggested_steps.includes("keep_operator_review_deferred_and_forbidden_quarantine_sources_out_of_execution"), true);
  assert.equal(payload.work_order.suggested_steps.includes("prove_intake_dataset_provider_and_bypass_counters_remain_zero"), true);
  assert.equal(payload.work_order.validation_commands.includes("npm --silent run hermes:source-intake-ledger-report"), true);
  assert.equal(payload.work_order.validation_commands.includes("npm --silent run hermes:source-intake-plan"), true);
  assert.equal(payload.work_order.validation_commands.includes("npm --silent run hermes:source-use-manifest"), true);
  assert.equal(payload.work_order.acceptance_criteria.includes("source_intake_ledger.total_records>=2"), true);
  assert.equal(payload.work_order.acceptance_criteria.includes("source_intake_ledger.top_allowed_contract=route:replay_backfill"), true);
  assert.equal(payload.work_order.acceptance_criteria.includes("intake_command_executed_count=0"), true);
  assert.equal(payload.work_order.acceptance_criteria.includes("dataset_fetch_attempted_count=0"), true);
  assert.equal(payload.work_order.acceptance_criteria.includes("provider_command_executed_count=0"), true);
  assert.equal(payload.work_order.acceptance_criteria.includes("bypass_attempted_count=0"), true);
  assert.equal(payload.work_order.prohibited_changes.includes("do not automate sportsbook browser sessions"), true);
  assert.equal(payload.work_order.prohibited_changes.includes("do not bypass anti-bot, geolocation, paywall or Terms-of-Service controls"), true);
  assert.equal(payload.implementation_policy.spend_provider_quota, false);
  assert.equal(payload.implementation_policy.real_execution_allowed, false);
  assert.equal(payload.evidence.sources.includes("source_intake_ledger"), true);
  assert.equal(payload.evidence.backlog_evidence.source_intake_ledger.top_allowed_contract, "route:replay_backfill");
  assert.equal(payload.evidence.backlog_evidence.source_intake_ledger.dataset_fetch_attempted_count, 0);
  assert.equal(payload.evidence.backlog_evidence.source_intake_ledger.provider_command_executed_count, 0);
  assert.equal(payload.evidence.backlog_evidence.source_intake_ledger.bypass_attempted_count, 0);
  assert.equal(payload.safety.sportsbook_bypass_allowed, false);
  assert.equal(payload.safety.browser_sportsbook_automation_allowed, false);
  assert.equal(payload.safety.anti_bot_bypass_allowed, false);
  assert.equal(payload.safety.credential_or_session_extraction_allowed, false);
...
test("learning-review summarizes model readiness without enabling real execution", async () => {
    ...
    "/api/v1/dashboard/live-state": {
      operational_state: {
        ...
        model_lab: {
          ...
          replay_backfill_seed_status: "ready",
          replay_backfill_seed_count: 37,
          closing_line_proxy_seed_ready: true,
          paper_learning_seed_ready: true,
          signal_gate_regression_ready: true,
          can_use_replay_backfill_for_rehearsal: true,
  ...
  try {
    ...
    assert.equal(payload.metrics.replay_backfill_seed_status, "ready");
    assert.equal(payload.metrics.replay_backfill_seed_count, 37);
    assert.equal(payload.metrics.closing_line_proxy_seed_ready, true);
    assert.equal(payload.metrics.paper_learning_seed_ready, true);
    assert.equal(payload.metrics.signal_gate_regression_ready, true);
    ...
    assert.equal(payload.gates.replay_backfill_rehearsal_seed_ready, true);
    assert.equal(payload.gates.can_promote_model_from_replay_seeds, false);
...
test("learning-review treats replay backfill seeds as rehearsal-only without production examples", async () => {
  const fixtures = eventRouterFixtures({
    "/api/v1/signals/live": [],
    "/api/v1/paper/performance": {
      readiness_status: "ready_for_review",
  ...
  try {
    ...
    assert.equal(result.exit, 0);
    const payload = JSON.parse(result.stdout);
    assert.equal(payload.review_status, "collecting");
    assert.equal(payload.gates.replay_backfill_rehearsal_seed_ready, true);
    assert.equal(payload.gates.can_promote_model_from_replay_seeds, false);
    assert.equal(payload.blockers.includes("live_backtest_dataset_not_ready"), true);
    assert.equal(payload.blockers.includes("replay_backfill_seeds_rehearsal_only"), true);
    assert.equal(payload.real_execution_recommendation, "keep_blocked");
    assert.equal(payload.safety.can_submit_real_orders, false);
  } finally {
    server.close();

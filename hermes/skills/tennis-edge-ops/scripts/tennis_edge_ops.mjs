...
function buildSourceDiscovery({ report, eventPlan }) {
  ...
  const sourceDossier = buildHistoricalBackfillSourceDossier();
  return {
    generated_at: new Date().toISOString(),
    ...
    discovery_scope: [
      ...
      "cc0_historical_match_backfill",
      "point_by_point_or_shot_backfill",
      "licensed_historical_odds_archive",
    ],
    acquisition_matrix: {
    ...
    backfill_source_dossier: sourceDossier,
    backfill_source_summary: {
      total_sources: sourceDossier.length,
      operator_review_required: sourceDossier.filter((source) => source.decision === "operator_review").length,
      no_commercial_clearance_sources: sourceDossier.filter((source) => source.commercial_clearance_required).length,
      cc0_candidates: sourceDossier.filter((source) => source.license_class === "cc0_candidate_verify").length,
      licensed_api_candidates: sourceDossier.filter((source) => source.license_class === "paid_api_terms").length,
    },
...
function buildHistoricalBackfillSourceDossier() {
  return [
    {
      id: "jeff_sackmann_atp",
      provider: "JeffSackmann/tennis_atp",
      source_type: "public_git_csv",
      source_url: "https://github.com/JeffSackmann/tennis_atp",
      license_class: "cc_by_nc_sa_operator_review",
      ...
      feature_targets: ["surface_elo_seed", "ranking_trend_features", "serve_return_priors", "BO5_history"],
      decision: "operator_review",
      cost_tier: "free_public_noncommercial",
      commercial_clearance_required: true,
      attribution_required: true,
      safe_use: ["offline_priors", "backtest_seed", "calibration_research_after_license_review"],
      prohibited_use: ["commercial_import_without_clearance", "redistribution_without_terms_review", "live_state_substitution"],
      required_operator_evidence: ["license_terms_reviewed", "attribution_plan", "local_source_manifest"],
    },
    {
      id: "jeff_sackmann_wta",
      provider: "JeffSackmann/tennis_wta",
      source_type: "public_git_csv",
      source_url: "https://github.com/JeffSackmann/tennis_wta",
      license_class: "cc_by_nc_sa_operator_review",
      license: "CC BY-NC-SA 4.0 per Tennis Abstract repository",
      coverage: ["WTA tour", "Grand Slam women"],
      feature_targets: ["grand_slam_women_priors", "surface_elo_seed", "ranking_trend_features"],
      decision: "operator_review",
      cost_tier: "free_public_noncommercial",
      commercial_clearance_required: true,
      attribution_required: true,
    ...
    },
    {
      ...
      source_type: "public_git_point_data",
      source_url: "https://github.com/JeffSackmann/tennis_slam_pointbypoint",
      license_class: "cc_by_nc_sa_operator_review",
      license: "CC BY-NC-SA 4.0 per Tennis Abstract repository",
      coverage: ["Australian Open", "Roland Garros", "Wimbledon", "US Open"],
      feature_targets: ["markov_point_engine_calibration", "tiebreak_and_pressure_features", "BO5_point_state_priors"],
      decision: "operator_review",
      cost_tier: "free_public_noncommercial",
      commercial_clearance_required: true,
      attribution_required: true,
      safe_use: ["offline_point_state_priors", "markov_rehearsal_after_license_review"],
      prohibited_use: ["commercial_import_without_clearance", "live_point_feed_replacement", "redistribution_without_terms_review"],
      required_operator_evidence: ["license_terms_reviewed", "attribution_plan", "local_source_manifest"],
    },
    {
      id: "match_charting_project",
      provider: "Tennis Abstract Match Charting Project",
      source_type: "public_git_charted_points",
      source_url: "https://github.com/JeffSackmann/tennis_MatchChartingProject",
      license_class: "cc_by_nc_sa_operator_review",
      license: "CC BY-NC-SA 4.0 per repository terms.",
      coverage: ["charted ATP/WTA matches", "shot and point annotations where available"],
      feature_targets: ["serve_pattern_priors", "return_depth_priors", "rally_length_priors", "pressure_point_style"],
      decision: "operator_review",
      cost_tier: "free_public_noncommercial",
      commercial_clearance_required: true,
      attribution_required: true,
      safe_use: ["offline_style_features_after_license_review", "model_feature_research"],
      prohibited_use: ["commercial_import_without_clearance", "redistribution_without_terms_review", "live_match_scraping"],
      required_operator_evidence: ["license_terms_reviewed", "attribution_plan", "local_source_manifest"],
    },
    {
      id: "tennis_data_results_odds",
      provider: "Tennis-Data.co.uk",
      source_type: "public_historical_results_odds_csv",
      source_url: "https://www.tennis-data.co.uk/data.php",
      license_class: "public_terms_operator_review",
      ...
      feature_targets: ["market_prior_features", "closing_line_proxy_baseline", "odds_bucket_calibration"],
      decision: "operator_review",
      cost_tier: "free_public_terms_review",
      commercial_clearance_required: true,
      attribution_required: true,
      safe_use: ["offline_odds_priors_after_terms_review", "closing_line_proxy_research"],
      prohibited_use: ["terms_restricted_scraping", "redistribution_without_terms_review", "live_odds_replacement"],
      required_operator_evidence: ["terms_reviewed", "attribution_plan", "local_source_manifest"],
    },
    {
      id: "kaggle_tennis_cc0_candidates",
      provider: "Kaggle community datasets",
      source_type: "public_dataset_catalog",
      source_url: "https://www.kaggle.com/search?q=tennis+ATP+WTA+dataset",
      license_class: "cc0_candidate_verify",
      license: "Dataset-specific license must be verified on Kaggle before import.",
      coverage: ["ATP/WTA match history depending on selected dataset"],
      feature_targets: ["offline_match_results_seed", "ranking_form_baseline", "dataset_crosscheck"],
      decision: "operator_review",
      cost_tier: "free_public_dataset_review",
      commercial_clearance_required: false,
      attribution_required: true,
      safe_use: ["offline_crosscheck_after_dataset_license_review"],
      prohibited_use: ["bulk_download_without_license_review", "account_automation", "live_state_substitution"],
      required_operator_evidence: ["dataset_id", "license_terms_reviewed", "download_method_approved", "local_source_manifest"],
    },
    {
      id: "theoddsapi_archive",
      provider: "TheOddsAPI",
      source_type: "licensed_api_archive",
      source_url: "https://api.the-odds-api.com/",
      license_class: "paid_api_terms",
      license: "Paid API terms; private use only, no redistribution.",
      coverage: ["provider-supported tennis markets"],
      feature_targets: ["archive_odds_comparison", "closing_line_proxy", "provider_crosscheck"],
      decision: "operator_review",
      cost_tier: "budget_paid_or_existing_key",
      commercial_clearance_required: false,
      attribution_required: false,
      safe_use: ["operator_triggered_archive_smoke", "quota_tracked_snapshot_import"],
      prohibited_use: ["quota_spend_without_operator", "redistribution", "browser_scraping"],
      required_operator_evidence: ["api_key_configured", "quota_reviewed", "terms_reviewed", "smoke_run_approved"],
    },
  ];
}
...
function buildHistoricalBackfillSources({ report, grandSlam }) {
  ...
  const dossierSources = buildHistoricalBackfillSourceDossier().map((source, index) => (
    historicalBackfillSource({
      ...
      coverage: source.coverage,
      useCases: source.feature_targets,
      status: source.decision,
      costTier: source.cost_tier,
      license: source.license,
      licenseClass: source.license_class,
      licenseReviewRequired: true,
      commercialClearanceRequired: source.commercial_clearance_required,
      attributionRequired: source.attribution_required,
      command: historicalBackfillCommandForSource(source),
      evidence: [
        ...source.required_operator_evidence,
        `current_grand_slam_status=${grandSlam?.status ?? "unknown"}`,
      ],
      blockers: historicalBackfillBlockersForSource(source),
      sourceUrl: source.source_url,
      sourceType: source.source_type,
      featureTargets: source.feature_targets,
      safeUse: source.safe_use,
      prohibitedUse: source.prohibited_use,
      requiredOperatorEvidence: source.required_operator_evidence,
    })
  ));
  return [
    historicalBackfillSource({
...
function historicalBackfillCommandForSource(source) {
  if (source.id === "theoddsapi_archive") return "npm --silent run hermes:budget-chain";
  return "Create a local source manifest; then run a future offline importer only after license review.";
}

function historicalBackfillBlockersForSource(source) {
  return dedupeStrings([
    ...(source.license_class === "public_terms_operator_review" ? ["terms_review_required"] : []),
    ...(source.commercial_clearance_required ? ["commercial_use_not_cleared"] : []),
    ...source.required_operator_evidence,
  ]);
}
...
function historicalBackfillSource({
  ...
  return {
    ...
    feature_targets: featureTargets,
    safe_use: safeUse,
    prohibited_use: prohibitedUse,
    required_operator_evidence: requiredOperatorEvidence,
    command,
    evidence,
    blockers,
    execute_now: false,

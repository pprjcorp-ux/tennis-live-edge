...
- `docs/enterprise-accuracy-track.md`: no-budget-limit accuracy roadmap for
  top-tier tennis feeds, replay-first enterprise contracts, and Hermes'
  supervisory role.
...
- Hermes channel recovery planning should consume `npm run hermes:channel-recovery-plan`;
  it is read-only and should summarize failed channel gates, required local
  env names without values, verification commands, and human-only steps before
  any cron/Telegram activation.
...
- Hermes autonomy effectiveness should consume
  ...
  The ops compiler, implementation handoff, scheduler rehearsal, and cron
  proposal must include this evidence so repeated blockers turn into reviewable
  work orders, never automatic actions.
